    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-800 dark:text-slate-200 font-sans">
      <main className="container mx-auto px-4 py-8 md:py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
          {/* Input Section */}
          <div className="bg-white dark:bg-slate-800/50 p-6 md:p-8 rounded-2xl shadow-lg">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label htmlFor="product-name" className="block text-sm font-medium text-slate-700 dark:text-slate-300">
                  Product Name
                </label>
                <input
                  type="text"
                  ...
                  className="mt-1 block w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-md shadow-sm placeholder-slate-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                  placeholder="e.g., Ergonomic Office Chair"
                />
              </div>

              <div>
                <label htmlFor="extra-info" className="block text-sm font-medium text-slate-700 dark:text-slate-300">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full flex justify-center items-center gap-2 py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-slate-900 focus:ring-indigo-500 disabled:bg-indigo-400 disabled:cursor-not-allowed transition-colors"
              >
          ...
          <div className="space-y-6">
            {isLoading && (
              <div className="flex flex-col items-center justify-center p-8 bg-white dark:bg-slate-800/50 rounded-2xl shadow-lg h-full">
                <Loader />
                <p className="mt-4 text-slate-500 dark:text-slate-400">AI is crafting your content...</p>
              </div>
            )}
            {generatedContent && (
            ...
            {!isLoading && !generatedContent && (
                 <div className="flex flex-col items-center justify-center text-center p-8 bg-white dark:bg-slate-800/50 rounded-2xl shadow-lg h-full">
                    <SparklesIcon className="w-16 h-16 text-slate-300 dark:text-slate-600" />
                    <h2 className="mt-4 text-xl font-semibold text-slate-700 dark:text-slate-200">Your content will appear here</h2>
                    <p className="mt-1 text-slate-500 dark:text-slate-400">Fill out the form and let the AI do the magic!</p>
                </div>
            )}
          </div>
        </div>
